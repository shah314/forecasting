...
for(i in 1:Nsim)
{
	...
	X <- snssde1d(N=99, x0=x_0, Dt=0.00001, drift=d, diffusion=s, method="euler", M=1)
...
p <- ggplot()
p <- p + geom_line(aes(x=1:199, y=exp(measure), color="Original Data"))
p <- p + geom_line(aes(x=200:253, y=exp(measure_test), color="Original Data TEST"))
p <- p + geom_line(aes(x=200:299, y=exp(sum_x), color="Predicted using SDE"))
p <- p + ylab("Value of the Stock")
p <- p + xlab("Time in Days")
p <- p + ggtitle("Stock Price Prediction using TDNGBM")
p <- p + geom_ribbon(aes(x=c(200:299), y = exp(sum_x), ymin=exp(lower), ymax=exp(upper)), linetype=2, alpha=0.1)
p
