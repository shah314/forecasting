# Author: Shalin Shah
# Bitcoin price forecasting using stochastic differential equations
...
# Read the data
msft <- read.table("/Users/shah/Desktop/BTC-USD.csv", sep=',', header=T)
measure <- msft$Close
measure <- measure[measure != "null"]
measure <- as.numeric(measure)
measure <- log(log(measure))

# the last 14 days are for testing
measure_test <- measure[350:363]

# The first 349 days are for training
measure <- measure[1:349]
...
for(i in 1:Nsim)
{
	...
	X <- snssde1d(N=13, x0=x_0, Dt=0.00001, drift=d, diffusion=s, method="euler", M=1)
...
#mean(abs(exp(exp(measure_test)) - exp(exp(sum_x))))
#cor(measure_test, sum_x)
...
p <- p + geom_line(aes(x=350:363, y=exp(exp(sum_x)), color="Predicted using SDE"))
p <- p + ylab("Value of the Stock")
p <- p + xlab("Time in Days")
p <- p + ggtitle("Bitcoin Price Prediction using TDNGBM")
p <- p + geom_ribbon(aes(x=c(350:363), y = exp(exp(sum_x)), ymin=exp(exp(lower)), ymax=exp(exp(upper))), linetype=2, alpha=0.1)
p